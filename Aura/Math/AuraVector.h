...
#error Define Matrix2 client memory layout
	...
	inline void convertMatrix4ToRowMajor(float matrixOut[4 * 4], mat4 const& in)
	{
		memcpy(matrixOut, in, sizeof(float) * 4 * 4);
	}
	...
	inline void convertMatrix3ToRowMajor(float matrixOut[3 * 3], mat3 const& in)
	{
		memcpy(matrixOut, &in, sizeof(float) * 3 * 3);
	}
	...
	inline void convertMatrix2ToRowMajor(float matrixOut[2 * 2], mat2 const& in)
	{
		memcpy(matrixOut, &in, sizeof(float) * 2 * 2);
	}
