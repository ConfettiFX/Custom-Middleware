...
def FindMSBuild17():
	...
	try:
		...
		proc = subprocess.Popen(["C:/Program Files (x86)/Microsoft Visual Studio/Installer/vswhere.exe","-version","15","-requires" ,"Microsoft.Component.MSBuild"], stdout=subprocess.PIPE)
...
def AddTestingPreProcessor(enabledGpuSelection):
	fileList = ["The-Forge/Common_3/OS/Interfaces/IOperatingSystem.h"]
...
def selectActiveGpuConfig(forgeDir, projRootFolder, projectName, runIndex):
	...
	if "Aura" in projectName or "Visibility" in projectName:
		filename = "/"+projRootFolder+"/src/GPUCfg/activeTestingGpu.cfg"
	else:
		filename = ""+projRootFolder+"/src/"+projectName+"/GPUCfg/activeTestingGpu.cfg"
...
def TestXcodeProjects(iosTesting, macOSTesting, iosDeviceId):
	...
	projects = GetFilesPathByExtension("./Ephemeris","app", True)
	iosApps = []
	osxApps = []
	appsToTest = []
...
def BuildXcodeProjects(skipMacos, skipIos, skipIosCodeSigning, skipDebugBuild, skipReleaseBuild, printXcodeBuild):
	...
	xcodeProjects = ["/Ephemeris/macOS Xcode/Ephemeris/Ephemeris.xcodeproj"]
	...
	for proj in xcodeProjects:
		...
		#foldername = proj.split(os.sep)[1]
...
def BuildLinuxProjects():
	...
	projsToBuild = GetFilesPathByExtension("./Ephemeris","workspace", False)
...
def TestLinuxProjects():
	...
	projsToTest = GetFilesPathByExtension("./Ephemeris","workspace", False)
	for projectPath in projsToTest:
		...
		for conf in configurations:					
			...
			for child in xmlRoot:
				if child.tag == "Project":
					if child.attrib["Name"] != "OSBase" and child.attrib["Name"] != "OS" and child.attrib["Name"] != "Renderer" and  child.attrib["Name"] != "SpirVTools" and child.attrib["Name"] != "PaniniProjection" and child.attrib["Name"] != "gainput" and child.attrib["Name"] != "ozz_base" and child.attrib["Name"] != "ozz_animation" and child.attrib["Name"] != "Assimp" and child.attrib["Name"] != "zlib" and child.attrib["Name"] != "LuaManager" and child.attrib["Name"] != "AssetPipeline" and child.attrib["Name"] != "AssetPipelineCmd" and child.attrib["Name"] != "ozz_animation_offline" and child.attrib["Name"] != "EASTL":
						ubuntuProjects.append(child.attrib["Name"])
...
def TestWindowsProjects(useActiveGpuConfig):
	...
	projects = GetFilesPathByExtension("./Ephemeris","exe",False)
	fileList = []
...
def BuildAndroidProjects():
	...
	projsToBuild = GetFilesPathByExtension("./Ephemeris","iml", False)
	for projectPath in projsToBuild:
		#get working directory (excluding the workspace in path)
		rootPath = os.sep.join(projectPath.split(os.sep)[0:-1])
		#save current work dir
...
def BuildWindowsProjects(xboxDefined, xboxOnly, skipDebug, skipRelease, printMSBuild, skipAura):
	...
	if len(projects) == 0:
		projects = GetFilesPathByExtension("./Ephemeris","sln",False)
