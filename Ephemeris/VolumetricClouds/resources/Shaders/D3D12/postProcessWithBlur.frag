...
Texture2D<float4> g_BlurTexture : register(t9);
...
float4 main(PSIn input) : SV_TARGET
{
  ...
  float4 blurCloudsResult = g_BlurTexture.Sample(g_LinearClampSampler, input.TexCoord);
  volumetricCloudsResult = lerp(blurCloudsResult * 0.5, volumetricCloudsResult, volumetricCloudsResult.a);
