...
float4 main(in PSIn input) : SV_TARGET
{
	...
	worldPos = mul(g_VolumetricClouds.m_DataPerEye[0].m_ProjToWorldMat, float4(NDC, 0.0, 1.0));
	worldPos /= worldPos.w;

	float3 viewDir = normalize(worldPos.xyz - g_VolumetricClouds.m_DataPerEye[0].cameraPosition.xyz);

	float3 firstHitRayPos = (viewDir * depth) + g_VolumetricClouds.m_DataPerEye[0].cameraPosition.xyz;
	prevUV = mul(g_VolumetricClouds.m_DataPerEye[0].m_PrevWorldToProjMat, float4(firstHitRayPos, 1.0));
