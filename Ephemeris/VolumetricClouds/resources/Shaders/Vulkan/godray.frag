...
vec4 HLSLmain(PSIn input0)
{
    ...
    vec4 sunPos = (((VolumetricCloudsCBuffer.g_VolumetricClouds).m_WorldToProjMat_1st)*(vec4(sunWorldPos, 1.0)));
    ...
    (finalIntensity += min(((VolumetricCloudsCBuffer.g_VolumetricClouds).CloudCoverage * float (5.0)), float (0.0)));
