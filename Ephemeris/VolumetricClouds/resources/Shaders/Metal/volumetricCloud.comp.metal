  ...
  // Get the point projected to the inner atmosphere shell
				   ...
				   thread float(& intensity), thread float(& atmosphericBlendFactor), thread float(& depth), float2 uv)
   {
    ...
    void main(uint3 GTid, uint3 Gid, uint3 DTid)
    {
        ...
        DstTexture.write(float4(intensity, atmosphereBlendFactor, depth, density), uint2(int2((DTid).xy)));
    };
...
texture3d<float> highFreqNoiseTexture,texture3d<float> lowFreqNoiseTexture,
				   ...
				   //texture2d<float> g_PrevFrameTexture,
           texture2d<float> g_LinearDepthTexture,
				   sampler g_LinearClampSampler,sampler g_LinearWrapSampler,sampler g_PointClampSampler,sampler g_LinearBorderSampler,constant volumetricCloud::Uniforms_VolumetricCloudsCBuffer & VolumetricCloudsCBuffer,texture2d<float, access::read_write> DstTexture) :
