...
float2 SharedData[MAX_WIDTH];
    void main(uint3 GroupId, uint3 GroupThreadId, uint GroupIndex)
    {
        ...
        float4 CurrentPixelValue = InputTex.read(uint2(localIndex, groupIndex) * 2).rgba;
        (SharedData[localIndex] = (CurrentPixelValue).ra);
        simdgroup_barrier(mem_flags::mem_threadgroup);
        float2 resultColor = ((float2)(SharedData[localIndex]) * (float2)(weight[0]));
        for (int i = 1; (i < 3); (i++))
        {
        ...
        (OutputTex.write(float4((resultColor).r, (CurrentPixelValue).gb, (resultColor).g), uint2(uint2(localIndex, groupIndex))));
    };
...
uint GroupIndex [[thread_index_in_threadgroup]],
    ...
    constant Compute_Shader::Uniforms_RootConstantScreenSize & RootConstantScreenSize [[buffer(UPDATE_FREQ_USER + 6)]])
{
    uint3 GroupId0;
    GroupId0 = GroupId;
