...
#define IGOR_FIX_MU0
    static float2 getTransmittanceUV(float r, float mu)
    {
        float uR, uMu;
    ...
    static void getTransmittanceRMu(float2 ScreenCoord, thread float(& r), thread float(& muS))
    {
        (r = (ScreenCoord).y);
        (muS = (ScreenCoord).x);
#ifdef TRANSMITTANCE_NON_LINEAR
    ...
    static float2 getIrradianceUV(float r, float muS)
    {
        float uR = ((r - Rg) / (Rt - Rg));
    ...
    static void getIrradianceRMuS(float2 ScreenCoord, thread float(& r), thread float(& muS))
    {
    ...
    static void getMuMuSNu(float2 ScreenCoord, float r, float4 dhdH, thread float(& mu), thread float(& muS), thread float(& nu))
    {
    ...
    static float limit(float r, float mu)
    {
    ...
    static float opticalDepth(float H, float r, float mu, float d)
    {
    ...
    static float3 analyticTransmittance(float r, float mu, float d)
    {
    ...
    static float phaseFunctionR(float mu)
    {
        return (((const float)(3.0) / ((const float)(16.0) * M_PI)) * ((float)(1.0) + (mu * mu)));
    };
	
    static float phaseFunctionM(float mu)
    {
    ...
    static float3 getMie(float4 rayMie)
    {
