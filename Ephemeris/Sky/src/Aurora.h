...
class AuroraParticle
{
  public:
    ...
    void update(float deltaTime)
    {
      ...
      acceleration = vec3(0.0f, 0.0f, 0.0f); // acceleration is reset since it HAS been translated into a change in position (and implicitely into velocity)	
    }
...
public:
  ...
  void satisfyConstraint()
  {
    ...
    vec3 correctionVector = gap * (1.0f - rest_distance / current_distance) * 0.5f; // The offset vector that could moves p1 into a distance of rest_distance to p2
    
    p1->offsetPosition(correctionVector); // correctionVectorHalf is pointing from p1 to p2, so the length should move p1 half the length needed to satisfy the constraint.
    p2->offsetPosition(-correctionVector); // we must move p2 the negative direction of correctionVectorHalf since it points from p2 to p1, and not p1 to p2.	
  }
...
public:    
    ...
    void Init(float width, float height, uint32_t num_particles_width, uint32_t num_particles_height)
    {
      ...
      for (uint32_t x = 0; x < num_particles_width; x++)
      {
        ...
        {
          vec3 pos = vec3(width * ((float)x / (float)num_particles_width),
                        height * ((float)y / (float)num_particles_height),
                          0.0f);
      ...
      */
      ...
      {
        getParticle(0 + i, 0)->offsetPosition(vec3(0.5f, 0.0f, 0.0f)); // moving the particle a bit towards the center, to make it hang more natural - because I like it ;)
        getParticle(0 + i, 0)->makeUnmovable();

        getParticle(num_particles_width - 1 - i, 0)->offsetPosition(vec3(-0.5f, 0.0f, 0.0f)); // moving the particle a bit towards the center, to make it hang more natural - because I like it ;)
        getParticle(num_particles_width - 1 - i, 0)->makeUnmovable();
      }
      */
    }

    AuroraParticle* getParticle(uint32_t x, uint32_t y)
    {
      return &particles[ y * numOfWidth + x];
    }

    void makeConstraint(AuroraParticle *p1, AuroraParticle *p2)
    {
      constraints.push_back(AuroraConstraint(p1, p2));
    }
    ...
    void update(float deltaTime)
    {
      ...
      {
        (*particle).update(deltaTime); // calculate the position of each particle at the next time step.
      }
    }
    ...
    eastl::vector<AuroraParticle>   particles; // all particles that are part of this cloth
    eastl::vector<AuroraConstraint> constraints; // alle constraints between particles as part of this cloth
