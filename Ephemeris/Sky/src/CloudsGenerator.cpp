...
void loadDistantClouds(ICloudsManager& CloudsManager, const DistantCloudParams& params)
{
    ...
    for (uint i = 0; i < params.cloudCount; i++)
    {
        ...
        transform = rotateZ(-CloudPosition[0] / 2000000.f * PI * 0.5f) * rotateX(CloudPosition[3] / 2000000.f * PI * 0.5f) *
                    rotateY(Orientation) * scale(CloudSize, CloudSize, CloudSize);
...
void generateCumulusCloudParticles(eastl::vector<vec4>& particlePosScale, eastl::vector<ParticleProps>& particleProps, int puffDimX = 10,
                                   int puffDimY = 10, float puffScaleMin = 1.f, float puffScaleMax = 1.f)
{
...
void loadCumulusClouds(ICloudsManager& CloudsManager, const CumulusCloudParams& params)
{
    ...
    {
        CloudsManager.createCumulusCloud(transform, NULL, ParticleScale, &particlePosScale[0], &particleProps[0],
