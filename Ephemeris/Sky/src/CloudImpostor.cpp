...
CloudImpostor::CloudImpostor():
    gFrameIndex(0), pRenderer(NULL), gImageCount(0), mWidth(0), mHeight(0), m_Radius(0), m_SortDepth(0), m_Valid(false), m_OldDistance(0)
{
    // m_TextureSize = 256;
    m_TextureSize = 512;
    // m_TextureSize = 1024;
...
float4 GetProjClampingRect(const float4x4& proj, const float4x4& view, float3 cameraCornerDirs[4], const float3& n, float d)
{
    float minX = -1.0f;
    float maxX = 1.0f;
    float minY = -1.0f;
...
void CloudImpostor::setupRenderer(Cmd* cmd, CumulusCloud* pCloud, const vec3& camPos, float camNear, const mat4& view, mat4& v, mat4& vp,
                                  vec3& dx, vec3& dy, ImpostorUpdateMode updateMode, bool& needUpdate
#ifdef CLAMP_IMPOSTOR_PROJ
                                  ,
...
)
{
    ...
    RenderTargetBarrier barriersImposter[] = { { m_tImpostor, RESOURCE_STATE_RENDER_TARGET } };
    ...
    if (distance > 0.0001)
    {
        ...
        */
        ...
        {
            m_ClampWindow = GetProjClampingRect(impostorProj, m_Transform, cameraCornerDirs, cloudDirUnit, distance);
...
#ifdef STABLISE_PARTICLE_ROTATION
        // pRenderer->setShaderConstant1f("masterParticleRotation", m_ParticleStabilize.updateMasterParticleRotation(vUp, vRight,
        // cloudDirUnit));
...
CloudImpostor::ParticleStabilizeData::ParticleStabilizeData():
    m_Valid(false), m_OldMasterParticleRotation(0.0f), m_MasterParticleRotation(0.0f)
{
}
