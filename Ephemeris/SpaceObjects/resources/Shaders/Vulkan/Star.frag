...
layout(set = 0, binding = 19) uniform texture2D volumetricCloudsTexture;
vec4 HLSLmain(PsIn In)
{
    ...
    float density = textureLod(sampler2D(volumetricCloudsTexture, g_LinearClamp), screenUV, 0.0f).a;
    ...
    return vec4(In.color, Mask * alpha * (1.0 - density));
}
void main()
{
    PsIn In;
