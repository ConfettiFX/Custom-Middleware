...
vec3 FXAA(vec2 UV, ivec2 Pixel)
{
    ...
    vec3 colorCenter = texelFetchOffset(SrcTexture, ivec2(Pixel.x, Pixel.y), 0, ivec2(0, 0)).rgb;
    float lumaCenter = rgb2luma(colorCenter);
    float lumaD = rgb2luma( texelFetchOffset(SrcTexture, ivec2(Pixel.x, Pixel.y), 0, ivec2(0, -1)).rgb );
    float lumaU = rgb2luma( texelFetchOffset(SrcTexture, ivec2(Pixel.x, Pixel.y), 0, ivec2(0, 1)).rgb );
    float lumaL = rgb2luma( texelFetchOffset(SrcTexture, ivec2(Pixel.x, Pixel.y), 0, ivec2(-1, 0)).rgb );
    float lumaR = rgb2luma( texelFetchOffset(SrcTexture, ivec2(Pixel.x, Pixel.y), 0, ivec2(1, 0)).rgb );
    ...
    {
        return vec3 ((texture(sampler2D( SrcTexture, g_LinearClamp), vec2(UV))).rgb);
    }
    float lumaDL = rgb2luma( texelFetchOffset(SrcTexture, ivec2(Pixel.x, Pixel.y), 0, ivec2(-1, -1)).rgb );
    float lumaUR = rgb2luma( texelFetchOffset(SrcTexture, ivec2(Pixel.x, Pixel.y), 0, ivec2(1, 1)).rgb );
    float lumaUL = rgb2luma( texelFetchOffset(SrcTexture, ivec2(Pixel.x, Pixel.y), 0, ivec2(-1, 1)).rgb );
    float lumaDR = rgb2luma( texelFetchOffset(SrcTexture, ivec2(Pixel.x, Pixel.y), 0, ivec2(1, -1)).rgb );
    ...
    float lumaEnd1 = rgb2luma(vec3 ((texture(sampler2D( SrcTexture, g_LinearClamp), vec2(uv1))).rgb));
    float lumaEnd2 = rgb2luma(vec3 ((texture(sampler2D( SrcTexture, g_LinearClamp), vec2(uv2))).rgb));
    ...
    if((!reachedBoth))
    {
        ...
        for (int i = 2; (i < 12); (++i))
        {
            if((!reached1))
            {
                (lumaEnd1 = rgb2luma(vec3 ((texture(sampler2D( SrcTexture, g_LinearClamp), vec2(uv1))).rgb)));
                (lumaEnd1 = (lumaEnd1 - lumaLocalAverage));
            }
            if((!reached2))
            {
                (lumaEnd2 = rgb2luma(vec3 ((texture(sampler2D( SrcTexture, g_LinearClamp), vec2(uv2))).rgb)));
